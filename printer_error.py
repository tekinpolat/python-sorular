...
the printer as a string representing a rational whose numerator is the number of errors and the denominator the length of the control string. Don't reduce this fraction to a simpler expression.
...
s="aaaxbbbbyyhwawiwjjjwwm"  # a-m  --> xyywwwww
...
def printer_error(s):
    hata_sayisi = 0
    for karakter in s:
        if not 'a' <= karakter  <= 'm':    # karakter > 'm'   -  karakter <'a' or karakter > 'm'   # 'y' > 'm' False or True
            hata_sayisi += 1
            
    return f'{hata_sayisi}/{len(s)}'

s="aaabbbbhaijjjm"
sonuc = printer_error(s)
print(sonuc)
